...
public class DFA extends FA {
    ...
    DFA (ArrayList<String> statesList, ArrayList<String> alphabetList, Map<String, String[][]> statesTransitions,
    ...
    public boolean isValid() {
        ...
        if (transitionFunction.size() != this.states.size()) { return false; }
        for (String state : this.states) {
            String[][] transitions = transitionFunction.get(state);
            for (String[] transition : transitions) {
                if (transition.length != this.alphabet.size() || !this.alphabet.contains(transition[0])
